...
ということで、満を持して以前から気になっていたElixir Phoenixを今回学んでみようと思います。

<!--more-->

## 注意
__今回はPhoenix部分は含まれておりません。__

Elixirの基本的な構文や制御文について触れていきます。
...
今回はRubyとの比較用のコードですが、もちろんこれがElixirっぽい書き方かと言われるとそうではなく、パターンマッチやマクロといった機能を使うのがElixirっぽいコードとしてはベターかと思われます。
...
例えばそれぞれの長さを知りたい場合、Listだと最初の要素から最後まで順番に要素にアクセスしていかなければならないため遅いですが、Tupleだと最初から一定の長さでメモリを確保しているため、長さを知るのは速いです。
...
一方で、値の追加や削除をする場合はListだと1つの番地を確保して、参照先を変更するだけで済みますが、Tupleは新しく連続した番地を確保しなければいけないため遅いです。 
...
#### cond
caseは最初に定義した１つの値が複数の値と比較してマッチするかどうかで分岐していましたが、condは複数の条件式を比較してtrueになったかどうかで分岐するという用途に使います。
...
しかしながら、これもパターンマッチの一種となりますので本記事では詳しくは扱いません。
