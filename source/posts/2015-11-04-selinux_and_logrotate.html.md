...
一通りコードを読み、ステージ環境でlogrotateコマンドを手動実行してアプリのログがローテートされたのを確認し、PRをマージしました。そして、プロダクション環境にcookしました。
...
ところでselinuxはいったい何に対して、どのようにアクセス制限を行っているのでしょうか。よくわかっていなかったので調べました。
...
具体的には、プロセスとディレクトリやファイルなどのリソースに対して「タイプ」というラベルを付与しています。<br>
