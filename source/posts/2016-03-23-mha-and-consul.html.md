...
なぜ、どのようにしてmhaとconsulを使った冗長化を行ったか、なんで1台増えてるのか、話していきます。
...
- マスターで障害発生した場合、復旧が大変
	- 元マスターを復旧しスレーブとして追加する際に、現マスターからMySQLのスナップショットを取る都合上ダウンタイム必須
...
[mha](https://code.google.com/p/mysql-master-ha/)とは複雑な操作や苦しみを伴うこと無く、マスターのフェイルオーバーを行えるツールです。  
マスターのダウン等による自動的なフェイルオーバーだけでなく、オンラインでのマスターサーバーの切り替えも行えます。(サーバーのメンテナンスしたい時に便利！)
...
さて、consulにはdns機能が内蔵されており、今回はこのdns機能とmhaを連携させることで **「マスターDBサーバーはどこか」** をconsulに問い合わせればわかるようにしました。
...
実際にDBマスターサーバーがダウンしてフェイルオーバーが行われる際、以下のようにしてこのスクリプトが実行されます。これはmhaのログで確認できます。(見やすさのため改行しています)
...
サンプルにある`FIXME_xxx;`を以下のように書き換えることで、consul(10.0.0.50)のapiを叩き、新マスターサーバーがどのホストなのかを知らせています。ここでmhaがスクリプトに渡していた引数(`new_master_ip`)が役に立ちます。
...
さて、consulのカタログデータベースにDBマスターサーバーのノードを登録すると、以下のように名前を引けるようになります。
...
dnsmasqを使って`*.consul`の名前の場合はconsulのDNSへ名前解決しにいくようにしました。
...
名前を引く → dnsmasq(127.0.0.1)に聞きに行く → `*.consul`ならconsul(10.0.0.50)へ、それ以外なら8.8.8.8へ名前解決しに行く
...
冒頭でさらっとサーバーの引っ越しをしましたと書いていますがかなり大変でした。大変でしたが、既存の仕組みをより保守しやすい仕組みに変えられたのでやる価値はありました！大きな変更を加えるにはこのタイミングしか無い、とチャンスを逃さなくてよかった。

今後はconsulをDBマスターサーバーへの名前引き以外でも活用していくぞ！！
