...
## 背景
弊社では社内にBINDでDNSサーバを立てており、各ドメインのネームサーバをそこに向けるようにしていました。ところが、最近の度重なるBINDの脆弱性が表面化し、セキュリティを担保する上でこのまま使い続けるのはリスクが高いという判断になりました。そこで、AWSのサービスであるRoute53に移転を検討しました。すでに一部サービスではRoute53を利用しておりましたが、設定がコンソールにしかなくその設定をした担当者しか経緯がわからないという状況があり、それを打開するためにも設定をコードに落としたいという思いがあってroadworkerを利用しようという話になりました。
